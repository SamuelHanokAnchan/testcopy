...
class SAMAutoDetection:
    ...
    def _initialize_sam(self):
        ...
        try:
            ...
            if "vit_b" in selected_model:
                ...
                logging.error(f"Unknown SAM model type in file: {selected_model}")
                self._setup_fallback()
                return
            ...
            try:
                ...
                logging.info(f"SAM model loaded: {selected_model} on {device}")
                
            except FileNotFoundError:
                logging.error(f"SAM model file not found: {checkpoint_path}")
                self._setup_fallback()
            ...
            logging.warning("SAM package not installed. Using basic computer vision detection.")
            logging.info("Install with: pip install git+https://github.com/facebookresearch/segment-anything.git")
            self._setup_fallback()
            
        except Exception as e:
            logging.warning(f"SAM initialization failed: {e}. Using fallback detection.")
            self._setup_fallback()
    ...
    def detect_with_points(self, image_array: np.ndarray, 
                          ...
                          point_labels: List[int] = None) -> List[List[Tuple[int, int]]]:
        """
    ...
    def _sam_detect_everything(self, image_array: np.ndarray, min_area: int, 
        ...
        try:
            ...
            logging.error(f"SAM detection failed: {e}")
            return self._fallback_detect_everything(image_array, min_area, max_areas)
    
    def _sam_detect_with_points(self, image_array: np.ndarray, 
                               point_coords: List[Tuple[int, int]], 
                               point_labels: List[int]) -> List[List[Tuple[int, int]]]:
        ...
        try:
            ...
            logging.error(f"SAM point detection failed: {e}")
            return self._fallback_detect_with_points(image_array, point_coords)
    ...
    def _fallback_detect_with_points(self, image_array: np.ndarray, 
                                    point_coords: List[Tuple[int, int]]) -> List[List[Tuple[int, int]]]:
